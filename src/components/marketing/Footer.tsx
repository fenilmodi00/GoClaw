import Link from "next/link"
import Image from "next/image"
import { ArrowRight } from "lucide-react"
import { Button } from "@/components/ui/button"

export function Footer() {
    return (
        <>
            {/* CTA Section */}
            <section className="py-20 px-4 text-center">
                <h2 className="text-2xl sm:text-3xl font-bold text-white mb-3 leading-tight">
                    Ready to automate
                    <br />
                    your workflows?
                </h2>
                <p className="text-sm text-gray-500 mb-6">
                    Deploy your first AI agent in under 60 seconds.
                </p>
                <Link href="/deploy">
                    <Button className="bg-orange-500 hover:bg-orange-600 text-white text-sm h-10 px-6 rounded-full gap-2">
                        Deploy your agent
                        <ArrowRight className="h-4 w-4" />
                    </Button>
                </Link>
                <div className="mt-5">
                    <p className="text-xs text-gray-600 mb-1">
                        Need a custom agent workflow?
                    </p>
                    <Link
                        href="#"
                        className="text-xs text-orange-500 hover:text-orange-400 transition-colors inline-flex items-center gap-1"
                    >
                        Talk to us — we&apos;ll build it for you →
                    </Link>
                </div>
            </section>

            {/* Footer */}
            <footer className="border-t border-white/[0.04] py-10 px-4 sm:px-6 lg:px-8">
                <div className="max-w-6xl mx-auto space-y-6">
                    {/* Top Row */}
                    <div className="flex items-center justify-between">
                        {/* Logo */}
                        <Link href="/" className="flex items-center gap-2">
                            <Image
                                src="/logo/GoClaw.png"
                                alt="GoClaw Logo"
                                width={24}
                                height={24}
                                className="h-6 w-auto object-contain"
                            />
                            <span className="text-sm font-bold text-white">GoClaw</span>
                        </Link>

                        {/* Social Icons */}
                        <div className="flex items-center gap-4">
                            {/* Discord */}
                            <a
                                href="#"
                                className="text-gray-600 hover:text-gray-300 transition-colors"
                                aria-label="Discord"
                            >
                                <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
                                    <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.095 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.095 2.157 2.42 0 1.333-.947 2.418-2.157 2.418z" />
                                </svg>
                            </a>
                            {/* X / Twitter */}
                            <a
                                href="#"
                                className="text-gray-600 hover:text-gray-300 transition-colors"
                                aria-label="X"
                            >
                                <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
                                    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                                </svg>
                            </a>
                        </div>
                    </div>

                    {/* Links Row */}
                    <div className="flex items-center gap-6">
                        {["Pricing", "Features", "Blog", "Terms", "Privacy"].map((link) => (
                            <Link
                                key={link}
                                href={link === "Pricing" ? "#pricing" : link === "Features" ? "#features" : "#"}
                                className="text-xs text-gray-600 hover:text-gray-300 transition-colors"
                            >
                                {link}
                            </Link>
                        ))}
                    </div>

                    {/* Copyright */}
                    <p className="text-xs text-gray-700">
                        © 2025 GoClaw
                    </p>
                </div>
            </footer>
        </>
    )
}
